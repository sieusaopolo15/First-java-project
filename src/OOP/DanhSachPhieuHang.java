...
public class DanhSachPhieuHang implements ThaoTac {
	private int n, a = 0;
	ChiTietPhieuNhapHang[] ctpnh;
	Scanner sc = new Scanner(System.in);
	
	//CONSTRUCTOR
	public DanhSachPhieuHang() {
	...
	public void Search_Loai(String loaixe) {
		System.out.println("\t\t\t\t\t\t======KẾT QUẢ======");
		System.out.println("\n==============================================================================================================================================================");
		System.out.format("|| %4s | %8s | %12s | %32s | %30s | %15s | %8s | %7s | %10s ||\n", "STT", "MÃ PHIẾU", "THƯƠNG HIỆU", "TÊN SẢN PHẨM", "NHÂN VIÊN PT NHẬP HÀNG", "LOẠI XE NHẬP VỀ", "SỐ LƯỢNG", "ĐƠN GIÁ", "THÀNH TIỀN");
		for(int i = 0; i < n; i++) {
			if(ctpnh[i].getLoaixe().indexOf(loaixe) != -1) {
				System.out.format("|| %4s |", i + 1);
				ctpnh[i].Xuat();
			}
		}
	...
	public void Search() {
		...
		while(true) {
			...
			System.out.println("\t\t\t\t\t3.Ấn phím 3 để tìm kiếm khách hàng theo loại xe");
			...
			switch(select) {
				...
				case 2:
					...
					break;
				case 3:
					System.out.print("Bạn cần tìm loại xe gì: ");
		...
		try {
			...
			try {
				while(true) {
					...
					ctpnh[i] = new ChiTietPhieuNhapHang(maphieu, tennv, thuonghieu, tensp, loaixe, sl, gia, thanhtien);
					i++;
				}
